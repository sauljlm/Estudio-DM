...
export default function AnimatedCard() {
  ...
  return (
    <div
      ...
      className="relative w-full h-[550px] md:h-[360px] rounded-2xl overflow-hidden bg-[#b8e7c8] mt-6 md:mt-8"
    >
      <FloatingVector
        ref={elements[0].ref}
        className="absolute -top-10 -right-20 w-96 h-96 z-20"
        size={256}
      />

      <FloatingVector
        ref={elements[1].ref}
        className="absolute -bottom-32 left-1/4 w-72 h-72 z-20"
      />

      <FloatingVector
        ref={elements[2].ref}
        className="absolute top-10 right-96 w-40 h-40 z-20"
      />

      {/* Cubos fondo */}
      <div className="absolute top-0 -left-16 lg:w-96 lg:h-96 w-48 h-48 z-10">
        <div ref={elements[3].ref} className="w-full h-full">
          <div className="bg-[#8dd9a7] w-full h-full rounded-2xl" />
        </div>
      </div>

      <div className="absolute -bottom-40 right-40 w-96 h-96 z-10">
